...
<h1 class="h1"><a  title="registered client" href="client/registered_clients2.php"><img src="../images/result2.gif" alt="" width="88" height="62" /></a>Registered Student(s)</h1>
<center>
  <p class="er">If Student's name is not found, implies that student has not been allocated yet or PRESS F5</p></center>
...
['p'])) { // Already been determined.
 ...
 $q = "SELECT * FROM `registration` WHERE `treated`='0'";
 $r = mysql_query ($q);
...
($_GET['s'])) {
 ...
 $start = 0;
  ...
  $sql="SELECT * FROM `registration`  WHERE `treated`='0' ORDER BY `child_name` ASC LIMIT $start,$display";
...
'#eeeeee'); // Switch the background
 ...
 echo '<tr bgcolor="' . $bg . '">';  ?>
    ...
    <td><a  title="view record" href="<?php echo 'view_record2.php?id='.$row['id'];?>"><img src="../images/search voter.gif" width="20" height="20" /></a></td>
...
if ($current_page != 1) {
 echo '<center><a href="registered_clients2.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="registered_clients2.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
