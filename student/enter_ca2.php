...
<center>
  <h4>Enter CA 2 Test Result Form</h4></center>
<?php
	$ass=$_GET['msg'];
...
$classid=$_GET['classid'];
	...
	if(strlen($score)==0){
		...
		if($score){
			...
			$sql="UPDATE `ca` SET `ca2_test` = '$score' WHERE `status`='1' AND `student_regno`='{$_GET['regno']}' AND `student_id`='{$_GET['id']}'  AND `subject_id`='{$_GET['subid']}' LIMIT 1";
...
<tr>
  <td height="51">Enter CA 2 Test Score</td><td><input type="text" name="score" size="3" /></td></tr>
...
echo "<h1><center><p class=\"er\">The Score of This Student's CA 2 Test on The Selected Subject Has Been Entered !!! Consult Administrators For Help<br/>

He/She Scored $ca2_test/5</p></center></h1>";
 } ?>
</div></div>
